...
import automatizacionavanzada.animations.FadeInLeftTransition1;
...
public class controllLogin implements Initializable {
    ...
    private void aksiLogin(ActionEvent event) {
        if (txtUsername.getText().equals("robbStark") && txtPassword.getText().equals("robbStark")) {
            config2 c = new config2();
            c.newStage(stage, lblClose, "formMenu.fxml", "Test App", true, StageStyle.UNDECORATED, false);
        } else {
            config2.dialog(Alert.AlertType.ERROR, "Error , Revisa usuario y contraseña");
        }
    }
